...
class KarteNotificationModule(reactContext: ReactApplicationContext) : NativeRNKRTNotificationModuleSpec(reactContext) {
  companion object {
  ...
  override fun registerFCMToken(fcmToken: String?) {
    fcmToken?.let {
      KarteApp.registerFCMToken(it)
    }
  }

  override fun canHandle(message: ReadableMap): Boolean {
    return MessageHandler.canHandleMessage(toData(message))
  }

  override fun handle(message: ReadableMap): Boolean {
    // NOP (iOS only)
    return true
  }

  override fun show(message: ReadableMap) {
    MessageHandler.handleMessage(reactApplicationContext, toData(message))
  }

  override fun track(message: ReadableMap) {
    // NOP (iOS only)
  }

  override fun retrieveURL(message: ReadableMap): String? {
