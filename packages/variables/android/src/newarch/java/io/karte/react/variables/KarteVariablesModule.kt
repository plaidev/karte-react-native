...
class KarteVariablesModule(reactContext: ReactApplicationContext) : NativeRNKRTVariablesModuleSpec(reactContext) {
  companion object {
  ...
  override fun fetch(promise: Promise) {
    Variables.fetch {
      if (it) {
        variables.clear()
  ...
  override fun trackOpen(keys: ReadableArray, values: ReadableMap?) {
  ...
  override fun trackClick(keys: ReadableArray, values: ReadableMap?) {
    val vars = (0 until keys.size()).mapNotNull {
      keys.getString(it)
    }.mapNotNull {
      variables[it]
  ...
  override fun getInteger(key: String, defaultValue: Double): Double {
    val variable = variables[key] ?: return defaultValue
    return variable.long(defaultValue.toLong()).toDouble()
  }
  ...
  override fun getArray(key: String, defaultValue: ReadableArray): WritableArray {
  ...
  override fun getObject(key: String, defaultValue: ReadableMap): WritableMap {
